...
class CommitFile(Artifact):
    ...
    @property
    def status(self):
        return self.raw_data['status'] if 'status' in self.raw_data else None

    @property
    def patch(self):
        return self.raw_data['patch']

    @property
    def changes(self):
        return self.raw_data['changes'] if "changes" in self.raw_data else (self.raw_data['change'] if "change" in self.raw_data else '')
